...
def detect_header(p, year):
    txt = norm(p.get('text', ''))[:2800]
    top = txt[:1600]
    cues = sum(k in top for k in ('anno', 'campinas', 'santuario', 'trindade'))
    month_alt = '|'.join(MONTH_NUM.keys())
    m = re.search(rf'(?<!\d)(\d{{1,2}})\s*(?:de\s*)?({month_alt})\s*(?:de\s*)?{year}(?!\d)', txt)
    if not m or cues < 2:
        return None
    day = int(m.group(1))
    ...
    issue = None
    areas = [txt[m.end():m.end()+550], top]
    patterns = [
        r'(?:numero|n[u]?mero)\s*[.:º°o-]*\s*(\d{1,4})',
        r'\bn\s*[º°o.]?\s*(\d{1,4})\b',
    ]
    for area in areas:
        for pat in patterns:
            z = re.search(pat, area)
            if z:
                n = int(z.group(1))
                if 1 <= n <= 9999:
                    issue = n
...
def build_editions(year, rawpages, annual_original):
    ...
    # Em 1929 e 1930 as datas ja estao gravadas nos indices gerados anteriormente.
    known = {}
    for p in pages:
    ...
    have = {h['iso'] for _, h, _ in starts}
    bypage = {gpage(p): p for p in pages}
    for iso, pg in known.items():
        if iso not in have and pg in bypage:
            starts.append((pg, {'iso': iso, 'issue': None}, bypage[pg]))
...
def datafiles(year):
    if year <= 1926:
        return ['search-index-all.json']
    if year == 1927:
        return ['search-index-1927.json']
    if year == 1928:
        return ['search-index-1928-p1.json', 'search-index-1928-p70.json', 'search-index-1928-p153.json']
...
def make_page(year, editions, pages):
    ...
    for seq, e in enumerate(editions, 1):
        ...
        buttons = [
            '<button class="btn read" data-id="' + html.escape(pdf) + '" data-start="' + str(e['start']) + '" data-end="' + str(e['end']) + '" data-title="' + html.escape(title) + '">Ler</button>'
        ]
        if pdf:
        ...
        cards.append(
            '<article class="doc-card"><div class="doc-head"><span class="badge">PDF</span><h2>' + html.escape(title) + '</h2><p>' + html.escape(dlab) + '</p><small>' + str(e['pages']) + ' página(s) indexada(s)</small></div><div class="doc-actions">' + ''.join(buttons) + '</div></article>'
        )
...
document.addEventListener('click',e=>{const b=e.target.closest('.read');if(!b)return;const id=b.dataset.id;ttl.textContent=b.dataset.title;frame.src=id?preview(id):'about:blank';renderOcr(Number(b.dataset.start),Number(b.dataset.end));tab('pdf');dlg.showModal()});$('close').onclick=()=>{frame.src='about:blank';dlg.close()};$('tabPdf').onclick=()=>tab('pdf');$('tabOcr').onclick=()=>tab('ocr');$('copyOcr').onclick=async()=>{const text=$('ocrText').innerText;try{await navigator.clipboard.writeText(text);$('copyOcr').textContent='Copiado!';setTimeout(()=>$('copyOcr').textContent='Copiar texto',1400)}catch(e){const r=document.createRange();r.selectNodeContents($('ocrText'));const sel=getSelection();sel.removeAllRanges();sel.addRange(r)}};
    ...
    return (page.replace('__YEAR__', str(year))
                .replace('__STYLE__', STYLE)
                .replace('__COUNT__', str(len(editions)))
                .replace('__PAGES__', str(len(pages)))
                .replace('__CARDS__', '\n'.join(cards))
                .replace('__FILES__', json.dumps(datafiles(year), ensure_ascii=False)))
...
def main():
    ...
    extra = {
        1927:['search-index-1927.json'],
        1928:['search-index-1928-p1.json','search-index-1928-p70.json','search-index-1928-p153.json'],
        1929:['search-index-1929.json'],
        1930:['search-index-1930.json'],
    }
    for y, files in extra.items():
        for f in files:
            allpages[y].extend(load_json(f))

    annual_original = {
        1929:'18ZmFkQPkRL5dSXhwXDbiXr34ifXDR0uo',
        1930:'1Ye6iDuiBClCLLSBhnpyV3lvoEcsKWHbk',
    }

    summary = {}
    editions_by_year = {}
    for y in range(1923, 1931):
        editions, pages = build_editions(y, allpages[y], annual_original)
        editions_by_year[y] = editions
        summary[str(y)] = {
            'edicoes_identificadas': len(editions),
            'paginas_indexadas': len(pages),
            'com_numero_impresso': sum(e['issue'] is not None for e in editions),
            'primeira_data': editions[0]['iso'] if editions else None,
            'ultima_data': editions[-1]['iso'] if editions else None,
        }
        (ROOT / f'ano-{y}.html').write_text(make_page(y, editions, pages), encoding='utf-8')

    doc = ROOT / 'documentacao' / 'PADRONIZACAO_ANOS_1923_1930.json'
    doc.parent.mkdir(exist_ok=True)
    doc.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')

    # Atualiza o resumo da pagina inicial para contar edicoes em vez de partes de PDF.
    ip = ROOT / 'index.html'
    if ip.exists():
        ...
        for y in range(1923, 1931):
            ...
            m = re.search(pat, s)
            if m:
                repl = f"{{year:{y},label:'Ano organizado em edições separadas',files:{n},folder:'{m.group(1)}'}}"
    ...
    if bad:
        raise SystemExit('Anos sem edicoes identificadas: ' + ', '.join(bad))

    for y in range(1923, 1931):
        t = (ROOT / f'ano-{y}.html').read_text(encoding='utf-8')
        if 'doc-card' not in t or 'Edição' not in t or 'Abrir arquivo' not in t:
            raise SystemExit(f'Falha na pagina {y}')
