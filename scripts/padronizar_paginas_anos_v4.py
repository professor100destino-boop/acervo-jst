...
def spaced(w): return r'\W*'.join(re.escape(c) for c in w)
...
def header_info(raw,year):
    ...
    issue=None
    for pat in (r'n\W*u\W*m\W*e\W*r\W*o\W*[:.º°o-]*\W*(\d{1,4})',r'\bn\W*[º°o.]\W*(\d{1,4})\b'):
        m=re.search(pat,top)
        if m:
            n=int(m.group(1))
    ...
    for mn,mi in MONTH_NUM.items():
        ...
        if not ds:continue
        try:c=f'{year:04d}-{mi:02d}-{int(ds):02d}';date.fromisoformat(c)
        except:continue
        if m.start()<pos:iso=c;pos=m.start()
...
def detect_starts(year,pages):
    bypg={gp(p):p for p in pages};kd=known_dates(pages,year);starts=[]
    if year in (1929,1930) and kd:
        # Datas conferidas anteriormente: preservar exatamente todas elas.
        for iso,pg in sorted(kd.items(),key=lambda x:x[1]):
            p=bypg[pg];hi=header_info(p.get('text',''),year) or {}
            starts.append({'page':pg,'iso':iso,'issue':hi.get('issue'),'direct':True})
        # só deduplica por página/data, nunca pelo número OCR.
        clean=[];sp=set();sd=set()
        for s in starts:
            if s['page'] in sp or s['iso'] in sd:continue
            sp.add(s['page']);sd.add(s['iso']);clean.append(s)
        return clean
    for p in pages:
        hi=header_info(p.get('text',''),year)
        if hi:starts.append({'page':gp(p),'iso':hi.get('iso'),'issue':hi.get('issue'),'direct':True})
    clean=[];sp=set();sn=set()
    for s in sorted(starts,key=lambda x:x['page']):
        if s['page'] in sp:continue
        if s['issue'] is not None and s['issue'] in sn:continue
        sp.add(s['page'])
        if s['issue'] is not None:sn.add(s['issue'])
        clean.append(s)
    return clean

def fill_physical_gaps(starts,maxpg):
    if not starts:return starts
    starts=sorted(starts,key=lambda x:x['page'])
    out=[]
    # páginas anteriores à primeira primeira-página detectada
    first=starts[0]['page']
    p=1
    while first-p>6:
        out.append({'page':p,'iso':None,'issue':None,'direct':False});p+=4
    out+=starts
    # inserir primeiras páginas omitidas em lacunas maiores que uma edição de 6 páginas
    enriched=[]
    for i,s in enumerate(out):
        enriched.append(s)
        if i+1>=len(out):continue
        nxt=out[i+1]['page'];cur=s['page']
        while nxt-cur>6:
            cur+=4
            if cur<nxt:enriched.append({'page':cur,'iso':None,'issue':None,'direct':False})
    out=sorted(enriched,key=lambda x:x['page'])
    # depois da última detectada
    cur=out[-1]['page']
    while maxpg-cur+1>6:
        cur+=4
        if cur<=maxpg:out.append({'page':cur,'iso':None,'issue':None,'direct':False})
    # garantir página 1 quando não houver começo próximo
    if out and out[0]['page']>2:out.insert(0,{'page':1,'iso':None,'issue':None,'direct':False})
    # dedupe
    d={s['page']:s for s in out}
    return [d[k] for k in sorted(d)]

def infer_sequence(starts,year):
    # Inferir intervalo editorial pelo índice da edição, não pela distância física de páginas.
    anchors=[(i,date.fromisoformat(s['iso'])) for i,s in enumerate(starts) if s.get('iso')]
    vals=[]
    for (i1,d1),(i2,d2) in zip(anchors,anchors[1:]):
        di=i2-i1
        if 1<=di<=12:
            r=(d2-d1).days/di
            if 5<=r<=17:vals.append(r)
    step=round(statistics.median(vals)) if vals else (14 if year==1923 else 7)
    step=14 if step>=11 else 7
    for i,s in enumerate(starts):
        if not s.get('iso') and anchors:
            ai,ad=min(anchors,key=lambda a:abs(a[0]-i))
            cand=ad+timedelta(days=step*(i-ai))
            if cand.year==year:s['iso']=cand.isoformat();s['date_inferred']=True
    # Inferir número impresso pela posição relativa ao número conhecido mais próximo.
    nanchors=[(i,s['issue']) for i,s in enumerate(starts) if s.get('issue') is not None]
    for i,s in enumerate(starts):
        if s.get('issue') is None and nanchors:
            ai,an=min(nanchors,key=lambda a:abs(a[0]-i))
            cand=an+(i-ai)
            if cand>0:s['issue']=cand;s['issue_inferred']=True
    return step

def build(year,raw,annual):
    pages=normalize_pages(raw);maxpg=max([gp(p) for p in pages],default=0)
    starts=detect_starts(year,pages)
    if year not in (1929,1930):starts=fill_physical_gaps(starts,maxpg)
    step=infer_sequence(starts,year)
    eds=[]
    for i,s in enumerate(starts):
        end=starts[i+1]['page']-1 if i+1<len(starts) else maxpg
        rel=[p for p in pages if s['page']<=gp(p)<=end]
        if not rel:continue
        eds.append({'issue':s.get('issue'),'iso':s.get('iso'),'date_inferred':s.get('date_inferred',False),'issue_inferred':s.get('issue_inferred',False),'start':s['page'],'end':end,'pages':len(rel),'pdf':rel[0].get('pdf') or '','original':rel[0].get('original') or annual.get(year) or ''})
...
def make_page(y,eds,pages):
    ...
    for seq,e in enumerate(eds,1):
        title=f"Edição nº {e['issue']}" if e.get('issue') is not None else f"Edição nº {seq}"
        ...
        if orig and orig!=pdf:b.append('<a class="btn secondary" href="https://drive.google.com/file/d/'+html.escape(orig)+'/view" target="_blank" rel="noopener">Original</a>')
        notes=[]
        if e.get('date_inferred'):notes.append('data reconstruída pela sequência editorial')
        if e.get('issue_inferred'):notes.append('número reconstruído pela sequência editorial')
        note=(' · '+'; '.join(notes)) if notes else ''
        cards.append('<article class="doc-card"><div class="doc-head"><span class="badge">PDF</span><h2>'+html.escape(title)+'</h2><p>'+html.escape(dlabel(e.get('iso')))+'</p><small>'+str(e['pages'])+' página(s) indexada(s)'+note+'</small></div><div class="doc-actions">'+''.join(b)+'</div></article>')
    t=r'''<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>JST __Y__ — Acervo Digital</title><style>__STYLE__</style></head><body><header class="top"><div class="wrap"><a class="back" href="index.html">← Voltar à pesquisa</a><div>ACERVO DIGITAL JST</div><h1>__Y__</h1><p class="lead">Ano completo em edições separadas.</p><div class="stats"><div class="stat"><strong>__N__</strong> arquivo(s)/edição(ões)</div><div class="stat"><strong>__P__</strong> páginas indexadas</div></div></div></header><main><div class="grid">__CARDS__</div></main><dialog id="dlg"><div class="dh"><strong id="ttl">Leitor</strong><button id="close">Fechar</button></div><div class="reader-tabs"><button id="tabPdf" class="reader-tab active">Jornal</button><button id="tabOcr" class="reader-tab">Texto OCR</button><button id="copyOcr" class="reader-copy hidden">Copiar texto</button></div><div id="pdfPane" class="reader-pane"><iframe id="frame"></iframe></div><div id="ocrPane" class="reader-pane hidden"><div id="ocrText" class="ocr-text"></div></div></dialog><script>let pages=[];const DATAFILES=__FILES__,YEAR=__Y__,$=id=>document.getElementById(id),dlg=$('dlg'),frame=$('frame'),ttl=$('ttl'),gp=p=>Number(p.globalPage||p.page||0),esc=x=>String(x??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));Promise.all(DATAFILES.map(f=>fetch(f+'?v='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():[]))).then(xs=>pages=xs.flat().filter(p=>Number(p.year||YEAR)===YEAR));function tab(o){$('pdfPane').classList.toggle('hidden',o);$('ocrPane').classList.toggle('hidden',!o);$('tabPdf').classList.toggle('active',!o);$('tabOcr').classList.toggle('active',o);$('copyOcr').classList.toggle('hidden',!o)}function render(s,e){let r=pages.filter(p=>gp(p)>=s&&gp(p)<=e);$('ocrText').innerHTML=r.map((p,i)=>'<article class="ocr-page"><h3>Página '+(i+1)+' da edição · página '+gp(p)+' do volume</h3><div>'+esc(p.text||'')+'</div></article>').join('')}document.addEventListener('click',e=>{let b=e.target.closest('.read');if(!b)return;ttl.textContent=b.dataset.title;frame.src=b.dataset.id?'https://drive.google.com/file/d/'+b.dataset.id+'/preview':'about:blank';render(+b.dataset.start,+b.dataset.end);tab(false);dlg.showModal()});$('close').onclick=()=>{frame.src='about:blank';dlg.close()};$('tabPdf').onclick=()=>tab(false);$('tabOcr').onclick=()=>tab(true);$('copyOcr').onclick=()=>navigator.clipboard&&navigator.clipboard.writeText($('ocrText').innerText);</script></body></html>'''
    return t.replace('__Y__',str(y)).replace('__STYLE__',STYLE).replace('__N__',str(len(eds))).replace('__P__',str(len(pages))).replace('__CARDS__','\n'.join(cards)).replace('__FILES__',json.dumps(datafiles(y)))
...
def main():
    ...
    for y in range(1923,1931):
        ...
        summary[str(y)]={'edicoes':len(eds),'paginas':len(pages),'numero_direto_ou_inferido':sum(e.get('issue') is not None for e in eds),'datas':sum(e.get('iso') is not None for e in eds),'datas_reconstruidas':sum(bool(e.get('date_inferred')) for e in eds),'numeros_reconstruidos':sum(bool(e.get('issue_inferred')) for e in eds),'intervalo_estimado_dias':step}
        (ROOT/f'ano-{y}.html').write_text(make_page(y,eds,pages),encoding='utf-8')
    (ROOT/'documentacao'/'PADRONIZACAO_ANOS_1923_1930.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
    ...
    ip.write_text(s,encoding='utf-8');print(json.dumps(summary,ensure_ascii=False,indent=2))
